...
def rotate_orientation(volume_data, volume_label, orientation=ORIENTATION['coronal']):
    if orientation == ORIENTATION['coronal']:
        ...
        raise ValueError("Invalid value for orientation. Please see help")
...
def remap_labels(labels, remap_config):
    ...
    elif remap_config == 'None':
        return labels
    else:
        raise ValueError("Invalid argument value for remap config, only valid options are FS, Neo and None")
